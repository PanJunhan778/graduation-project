...
public class AsyncConfig implements AsyncConfigurer {
    ...
    @Bean("homeAiSummaryTaskExecutor")
    public Executor homeAiSummaryTaskExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(2);
        executor.setMaxPoolSize(4);
        executor.setQueueCapacity(50);
        executor.setThreadNamePrefix("ems-home-ai-");
        executor.initialize();
        return executor;
    }

    @Bean("aiChatStreamTaskExecutor")
    public Executor aiChatStreamTaskExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(4);
        executor.setMaxPoolSize(8);
        executor.setQueueCapacity(100);
        executor.setThreadNamePrefix("ems-ai-stream-");
        executor.initialize();
        return executor;
    }
